...
class Channels:
    ...
    * TikHub 的置顶标记藏在 `show_tags_v2` 里，名字不叫 is_pinned，但确实有，
      而且实测准（2026-08、2026-09 各验过一次）
    * ⚠️ **备胎不是等价替身**：SocialDataX 的 `is_pinned` 实测是错的——对真正
      置顶的评论也返回 false（2026-09-21，同一条 comment_id 两家对打），
      而且它把评论正文里的蓝词标记 `#词[搜索高亮]#` 也抹平了。所以走备胎的
      那几轮，「置顶状态」按设计**完全不碰**（见 providers.XHS_COMMENT_CAPABILITIES），
      蓝词也扫不到。降级保的是「评论数/评论内容/负面词这条主线不断」，
      不是「什么都不少」——主通道余额别断，这是省不掉的运维动作
